...
<div class="container py-4">
    ...
    <a href="{{ route('admin.shops.show', $shop) }}" class="btn btn-sm btn-outline-primary me-1">Voir</a>

    @if($shop->statut === 'en_attente')
        <form action="{{ route('admin.shops.validate', $shop) }}" method="POST" class="d-inline">
            @csrf
            <button type="submit" class="btn btn-sm btn-outline-success me-1">Valider</button>
        </form>

        <form action="{{ route('admin.shops.refuse', $shop) }}" method="POST" class="d-inline">
            @csrf
            <button type="submit" class="btn btn-sm btn-outline-danger me-1">Refuser</button>
        </form>
    @else
        <a href="{{ route('admin.shops.edit', $shop) }}" class="btn btn-sm btn-outline-warning me-1">Modifier</a>
        <form action="{{ route('admin.shops.destroy', $shop) }}" method="POST" class="d-inline" onsubmit="return confirm('Supprimer cette boutique ?')">
            @csrf
            @method('DELETE')
